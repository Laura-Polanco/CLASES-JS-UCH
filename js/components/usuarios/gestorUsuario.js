...
class GestorUsuarios {
    ...
    renderizarUsuarios(usuarios) {
        ...
        usuarios.forEach(usuario => {
            const color = usuario.age < 18 ? "red" : "green"; // cambio edad por age
            $('#mainlogin table').append(`
                <tr>
                    <td>${usuario.name}</td>
                    <td style="color:${color};">${usuario.age}</td> // cambio edad por age
                    <td>${usuario.dni}</td>
                    <td>${usuario.estado}</td> // cambio estadoCivil por estado
                    <td class="usuario"><img src="${usuario.foto}" alt="" style="width: 100px; height: 100px;"></td> <!-- ajusta el ancho y alto aquí -->
                </tr>
            `);
        });
